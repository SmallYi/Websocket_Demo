...
showcolor = ['#99CC66','#0099CC','#FF6666','#CC0033']
level = ['轻微危险','一般危险','较高危险','极度危险']
...
class EvaluateHandler(tornado.web.RequestHandler):
    ''' 接受服务器端脚本提交的最新evaluate realtime行内容 '''
    label_five=['normal.']
    global level
    global showcolor
    def post(self, *args, **kwargs):
        ...
        if data:
           data_dict=json.loads(data)
           data_dict['showdata_FAHP'] = []
           data_dict['showdata_SQF'] = []
           n = len(data_dict['data']['eva_result_FAHP'])
           for i in range(0,n):
               data_dict['showdata_FAHP'].append({'value':data_dict['data']['eva_result_FAHP'][i],'name':level[i],\
               'itemStyle':{'color':showcolor[i]}})
               data_dict['showdata_SQF'].append({'value':data_dict['data']['eva_result_SQF'][i],'name':level[i],\
               'itemStyle':{'color':showcolor[i]}})
           print(data_dict)
           self.application.rtD.makelines(data_dict)
